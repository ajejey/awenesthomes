...
import { getProperty, updateProperty } from './actions';

// Define the steps in the property editing process
const steps = [
...
// Define the form schema using Zod (same as create property)
...
export default function EditPropertyPage({ params }: { params: { id: string } }) {
  ...
  const { handleSubmit, reset, formState: { isSubmitting, isValid, errors } } = methods;
    ...
    const fetchPropertyData = async () => {
      try {
        ...
        const transformedData = {
          ...data,
          availability: data.availability.map((range: any) => ({
            ...range,
            startDate: new Date(range.startDate),
            endDate: new Date(range.endDate),
          })),
          blockedDates: data.blockedDates?.map((range: any) => ({
            ...range,
            startDate: new Date(range.startDate),
            endDate: new Date(range.endDate),
          })) || [],
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex justify-center items-center h-64">
          ...
          <span className="ml-3 text-lg text-gray-600">Loading property data...</span>
        </div>
      </div>
    );
  }
