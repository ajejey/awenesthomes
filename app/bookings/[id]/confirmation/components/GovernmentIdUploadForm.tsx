...
import { z } from 'zod';
import { uploadGovernmentId } from '@/app/users/actions';
import { 
  CheckCircleIcon, 
...
type GovernmentIdFormData = z.infer<typeof governmentIdSchema>;

interface GovernmentIdUploadFormProps {
  userId: string;
  onSuccess: () => void;
}

export default function GovernmentIdUploadForm({ userId, onSuccess }: GovernmentIdUploadFormProps) {
  ...
  const onSubmit = async (data: GovernmentIdFormData) => {
    ...
    try {
      ...
      const result = await uploadGovernmentId(formData);
